<?php
/**
 * QueueRecoveryDepartmentTask Test Case
 *
 */
...
App::uses('QueueRecoveryDepartmentTask', 'Console/Command/Task');

/**
 * QueueRecoveryDepartmentTaskTest class
 *
 */
class QueueRecoveryDepartmentTaskTest extends AppCakeTestCase
{
    ...
    public function testRunTreeBroken()
    {
        ...
        $this->_targetObject->QueuedTask->createJob('RecoveryDepartment', null, null, 'recovery');
        $capabilities = [
            'RecoveryDepartment' => [
                'name' => 'RecoveryDepartment',
                'timeout' => RECOVER_TREE_DEPARTMENT_EXTENSION_TIME_LIMIT,
                'retries' => 2,
            ]
        ];
    ...
    public function testRunRecoveryNotRequired()
    {
        ...
        $failureMessage = Hash::get($taskInfo, 'QueuedTask.failure_message');
        $expected = __('The recovery tree of departments is not required');
        $this->assertData($expected, $failureMessage);
    }
}
