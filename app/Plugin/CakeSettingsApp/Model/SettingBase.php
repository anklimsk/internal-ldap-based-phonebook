...
class SettingBase extends CakeSettingsAppAppModel
{
    ...
    public function processGroupAction($groupAction = null, $conditions = null)
    {
        ...
        switch ($groupAction) {
            case 'group-data-del':
                $result = $modelExtendQueuedTask->deleteAll($conditions, false);
                break;
        }
    ...
    public function getBarStateInfo()
    {
        ...
        $fields = [
            'count(*) AS amount',
            $modelExtendQueuedTask->alias . '.status'
        ];
        $group = $modelExtendQueuedTask->alias . '.status';
        $order = [$modelExtendQueuedTask->alias . '.status' => 'asc'];
        $modelExtendQueuedTask->recursive = -1;
        $data = $modelExtendQueuedTask->find('all', compact('fields', 'group', 'order'));
        if (empty($data)) {
            return $result;
        }
        ...
        foreach ($data as $dataItem) {
            $stateId = $dataItem[$modelExtendQueuedTask->alias]['status'];
