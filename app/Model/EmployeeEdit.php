 ...
 * @param int $tnW New width of image file.
 * @param int $tnH New height of image file.
 ...
 */
	public function resizePhoto($filename = null, $tnW = PHOTO_WIDTH, $tnH = PHOTO_HEIGHT, $quality = 75) {
		$quality = (int)$quality;
		if ($quality < 0) {
			$quality = 75;
		}
		...
		if (empty($filename) || !file_exists($filename) ||
			!is_file($filename) || empty($tnW) || empty($tnH)) {
			return false;
		}
		...
		if (($srcW <= $tnW) && ($srcH <= $tnH)) {
			$newW = $srcW;
			$newH = $srcH;
		} elseif (($xRatio * $srcH) < $tnH) {
			$newH = ceil($xRatio * $srcH);
			$newW = $tnW;
		} else {
			$newW = ceil($yRratio * $srcW);
			$newH = $tnH;
		}

		$newpic = imagecreatetruecolor(round($newW), round($newH));
		imagecopyresampled($newpic, $source, 0, 0, 0, 0, $newW, $newH, $srcW, $srcH);
		$final = imagecreatetruecolor($tnW, $tnH);
		$backgroundColor = imagecolorallocate($final, 255, 255, 255);
		imagefill($final, 0, 0, $backgroundColor);
		imagecopy($final, $newpic, (($tnW - $newW) / 2), (($tnH - $newH) / 2), 0, 0, $newW, $newH);
